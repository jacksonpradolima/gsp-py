...
def test_gsp_pattern_hierarchy(transactions: List[List[str]]) -> None:
    """
    Property: Patterns at level k should generally have higher or equal min support
    than patterns at level k+1.
    
    As we go to longer patterns, the support should generally decrease or stay the same,
    since longer patterns are more specific.
    """
    gsp = GSP(transactions)
    ...
    for level_idx in range(len(result) - 1):
        current_level = result[level_idx]
        next_level = result[level_idx + 1]
        
        # The minimum support in the current level should be >= minimum in next level
        if current_level and next_level:
            min_support_current = min(current_level.values())
            min_support_next = min(next_level.values())
            
            assert min_support_current >= min_support_next, (
                f"Level {level_idx + 1} has min support {min_support_current}, "
                f"but level {level_idx + 2} has min support {min_support_next}, "
                f"which is higher"
            )
...
@given(
    transactions=transaction_lists(),
    min_support=st.floats(min_value=0.01, max_value=0.99)
)
@settings(max_examples=50, deadline=None, suppress_health_check=[HealthCheck.too_slow])
